		...
		<md-card>
			<md-card-header>
				...
				<md-card-header-text>
					...
					<span style="font-size:12px">{{now | date : 'medium'}}</span>
				</md-card-header-text>
			</md-card-header>
			<md-card-content>
				<div class="input-field col s12" style="margin-top:-20px">
					<form>
						<textarea id="textarea1" class="materialize-textarea" ng-model="content.text" ng-change="postLable=content.text.length"></textarea>
						<label for="textarea1" ng-init="postLable='What\'s in your mind? Create new post...'">{{postLable}}</label>
						<a class="btn-floating blue"><i class="material-icons">add_a_photo</i></a>
						<button type="button" ng-click="createPost()" ng-class="{disabled:!content.text}" class="btn blue right">Publish <i class="material-icons right">send</i></button>
					</form>
		        </div>
			</md-card-content>
		</md-card>
